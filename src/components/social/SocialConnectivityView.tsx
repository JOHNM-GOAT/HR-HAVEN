...
export const SocialConnectivityView: React.FC = () => {
  ...
  // Eligible colleagues for recipient quick-pick
  const availableColleagues = accounts.filter(
    a => a.email.toLowerCase() !== userProfile.email.toLowerCase() && a.status === 'active'
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className={`text-xs mt-1 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
            Celebrate peer contributions in real-time, send gratitude badges, and confidentially alert HR.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {(userRole === 'hr_manager' || userRole === 'admin') && (
            <button
              ...
              className={`flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl border text-xs font-bold transition-all shadow-xs active:scale-95 cursor-pointer ${
                isDarkMode 
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-full border text-xs font-bold transition-all shadow-sm active:scale-95 cursor-pointer ${
              showHrSuggest
                ? isDarkMode
      ...
      {showHrSuggest && (
        <div className={`enterprise-card p-6 sm:p-7 border shadow-xl rounded-2xl animate-fade-in relative overflow-hidden ${
          isDarkMode
          ...
          <p className={`text-xs mb-5 leading-relaxed font-medium ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>
          ...
          {submittedSuccess ? (
            <div className={`p-4 rounded-xl border flex items-center justify-between ${
              isDarkMode ? 'bg-emerald-950/60 border-emerald-500/40 text-emerald-300' : 'bg-emerald-50 border-emerald-300 text-emerald-900'
            }`}>
            ...
            <form onSubmit={handleHrSuggest} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className={`w-full rounded-xl px-4 py-3 text-xs font-medium transition-all border ${
                    isDarkMode
              ...
              <div>
                ...
                <textarea
                  ...
                  className={`w-full rounded-xl px-4 py-3 text-xs font-medium transition-all border resize-none h-24 ${
                    isDarkMode
                      ? 'bg-[#0f1118] border-[#2e323e] text-white placeholder-slate-500 focus:outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-900/40'
                      : 'bg-white border-slate-300 text-slate-900 placeholder-slate-400 shadow-xs focus:outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-200'
                  }`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className={`enterprise-card p-6 border flex flex-col justify-between ${
          isDarkMode ? 'bg-[#202229] border-[#2e323d] text-white' : 'bg-white border-slate-200 text-slate-900'
        }`}>
          <div>
            <div className="flex items-center gap-2 mb-4">
              <div className={`p-2 rounded-xl border ${
                isDarkMode ? 'bg-amber-950/60 text-amber-400 border-amber-800' : 'bg-amber-50 text-amber-600 border-amber-100'
              }`}>
              ...
              <div>
                ...
                <p className={`text-[11px] ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                  Recognize teammates and sync live to the database
                </p>
              </div>
            </div>
            ...
            <form onSubmit={handleSendBadge} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className={`w-full rounded-xl px-4 py-2.5 text-xs transition-all border ${
                    isDarkMode 
                ...
                {/* Quick Selection Chips from Directory */}
                {availableColleagues.length > 0 && (
                  <div className="flex items-center gap-1.5 mt-2 overflow-x-auto pb-1">
                    <span className={`text-[10px] shrink-0 font-medium ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>
                      Quick pick:
                    </span>
                    {availableColleagues.slice(0, 4).map(c => (
                      <button
                        key={c.id}
                        type="button"
                        onClick={() => setRecipient(c.name)}
                        className={`text-[10px] px-2 py-0.5 rounded-md border font-medium transition-all shrink-0 cursor-pointer ${
                          recipient === c.name
                            ? 'bg-amber-500 text-white border-amber-600'
                            : isDarkMode
                              ? 'bg-[#181a22] border-[#2e323e] text-slate-300 hover:text-white'
              ...
              <div>
                ...
                <textarea
                  ...
                  className={`w-full rounded-xl px-4 py-2.5 text-xs transition-all border resize-none h-24 ${
                    isDarkMode 
                      ? 'bg-[#181a22] border-[#2e323e] text-white placeholder-slate-500 focus:border-amber-500' 
                      : 'bg-slate-50 border-slate-200 text-slate-900 placeholder-slate-400 focus:border-amber-500 focus:bg-white'
                  }`}
              ...
              >
                ...
                <div className={`w-4 h-4 rounded border flex items-center justify-center ${
                  sendCoffee ? 'bg-amber-500 border-amber-500' : isDarkMode ? 'border-slate-600' : 'border-slate-300'
                }`}>
        ...
        <div className={`enterprise-card p-6 border lg:col-span-2 flex flex-col h-[600px] ${
          isDarkMode ? 'bg-[#202229] border-[#2e323d] text-white' : 'bg-white border-slate-200 text-slate-900'
        }`}>
          {/* Feed Title & Badges Filter Header */}
          <div className={`flex flex-col gap-3 pb-3 mb-3 border-b shrink-0 ${
            isDarkMode ? 'border-[#2e323e]' : 'border-slate-100'
          }`}>
              ...
              <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0">
                {(['all', 'lifesaver', 'focus_champion', 'team_anchor', 'positive_energy'] as const).map(f => (
                  <button
                    ...
                    className={`text-[11px] px-2.5 py-1 rounded-full font-bold transition-all shrink-0 cursor-pointer ${
                      activeFilter === f
            ...
            <div className={`flex flex-wrap items-center justify-between gap-2 pt-2 border-t ${
              isDarkMode ? 'border-[#2a2e3b]' : 'border-slate-100'
            }`}>
              ...
              filteredBadges.map(badge => (
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className={`text-xs font-bold px-3 py-1 rounded-full border flex items-center gap-1.5 shrink-0 ${
                      isDarkMode 
